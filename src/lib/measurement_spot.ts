...
export class MeasurementSpot {
    ...
    public calculateStatus = () => {
        ...
        this._status.rainDropAmplitudeMax = rainDropAmplitudes
            .map((drop) => drop.range)
            .reduce(max);
        this._status.rainDropAmplitudeMin = rainDropAmplitudes
            .map((drop) => drop.range)
            .reduce(min);
        this._status.rainDropAmplitudeCount = rainDropAmplitudes.length;
        this._status.rainDropAmplitudeCycleCount = rainDropAmplitudes
            .map((drop) => drop.cycleType)
